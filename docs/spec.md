...
- [x] 2.8 **`awaiting_choice` 进 `/state`**：「游戏正等你做选择」是一等一的
      ...
      不明说上层就只能猜。已实机验证（见 §3.7 结论；2026-08-01 商店除卡
      再次复现：买服务后 `awaiting_choice` 转 true 并带出 11 张可选牌）
- [x] 2.4a 地图可走节点 → `/state` 的 `map` 段（见 3.4b）
- [x] 2.4b 非战斗状态 → 统一为 `/state` 的 `screen` 段：奖励、卡牌三选一、
      休息点、宝箱、事件、商店（带价格）、主菜单、游戏结束，均已导出。
      **上述界面全部已实机验证**（商店 2026-08-01 验证，当场揪出三个 bug，
      见 §3.4f 结论）。原先额外列出的「Boss 遗物三选一」并不存在于
      v0.107.1，实机与反编译证据见 §3.4g
...
- [x] 3.4g ~~**Boss 遗物三选一**~~ → **取消：v0.107.1 没有这项机制**。
      2026-08-01 第三局实机击败第一章 Boss 后，奖励只有金币与稀有卡，按继续
      直接进入第二章，没有遗物界面。反编译同版 `RewardsSet.GenerateRewardsFor`
      复核：Boss 分支只加金币、概率药水与卡牌；`RelicRarity` 也没有 `Boss`。
      原任务是把一代机制误写进了二代清单，不再保留一个永远无法完成的假缺口。
      详见下方「3.4g 结论」
...
（见下方 2.8）。已实机验证：未决选择期间下发打击 → `ok:false`、
`error:"awaiting_choice"`，且那张打击原封不动还在手里（见 §3.7 结论）。
...
#### 3.4g 结论：能量构筑打过 Boss；但 v0.107.1 根本没有 Boss 遗物三选一

2026-08-01 用 6.4 的 runner 连打两局（力士 A2），都打到了第 16~17 层的
`LagavulinMatriarch`（222 血，`PlatingPower 12` 每回合回满格挡，
`AsleepPower 3` 开局睡三回合），**两次都输在最后一两个回合**：

```
第一局   Boss 剩 16 血时我方阵亡（我 11 血，它 14 点意图，手上无格挡牌）
        当时缺的是 1 点能量：两张完美打击 66 点够杀，但要 4 费而我只有 3 费
第二局   Boss 剩 33 血时我方阵亡（我 15 血，它 21 点意图，虚弱化下只挡得出 3 点）
        这一局已经做了改进：除掉诅咒、打铁升完美打击（22→30）、
        消亡粉末（每回合 -9 血）、肌肉药水凑爆发
```

两次都不是操作失误，是算得出的数值差距 —— 每回合能投送的伤害追不上它的
血量与成长。第三局据此把构筑优先级改成降费/回能，核心包括升级放血（0 费，
失去 3 生命、获得 3 能量）、战鼓、破灭与遗忘仪式。Boss 战中实测出现两次
...
`act=2, floor=0`，没有任何遗物选择。随后对正在运行的 v0.107.1 程序集重新
反编译核对：

- `RewardsSet.GenerateRewardsFor` 的 `RoomType.Boss` 分支只产生金币、概率药水、
  三选一卡牌；没有 `RelicReward` 或遗物选择命令；
- `RelicRarity` 枚举只有 Starter/Common/Uncommon/Rare/Shop/Event/Ancient，
  没有 Boss 稀有度；
- `NChooseARelicSelection` 的注释与调用点都指向 Jewelry Box 一类局外生成器，
  不是 Boss 奖励。

因此 3.4g 不是「实现了但未验证」，而是**把一代的 Boss 遗物机制错误投射到
二代 v0.107.1**。清单按不适用关闭；这不等于事件触发的
`NChooseARelicSelection` 已实机验证，日后实际遇到该事件时仍应按新界面处理。
...
- [x] 5.3 工具：`get_state` / `get_glossary` / `play_card` / `end_turn` /
      `use_potion` / `health`
- [x] 5.4 **`auto_play_until(stop_on=[...])`** —— 由 6.4 的 `auto_run` 实现，
      形状略有不同：停止条件不由调用方传入，而是写死在 runner 里并**默认交还**
      （认不出的一律停手）。理由见 6.4 结论 ——「该停在哪」本身就是策略，
      让调用方每次传一遍，等于把同一份策略抄在两个地方
...
- [x] 6.3a **策略沉淀** → `docs/strategy.md`。只收录实战验证过的判断，
      ...
      导不出的信息就是做不出的决策
- [x] 6.3b **开新局前复盘上一局** → `journal.brief()` / `record_plan()` /
      `has_plan()`，MCP 工具 `get_brief`（只递上一局的硬事实：死亡楼层、
      构筑取舍与停手原因）与 `set_plan`（只记模型自己写下的这一局要改什么），
      外加 `auto_run` 上一个新的停手点：新局第 1 层且本局尚无计划就交还。
      仍不把 strategy.md 复制成一套 prompt 模板——它会很快与原文漂移。
      见下方「6.3b 结论」
...
- [x] 6.4c **待选物的名字与效果文本** → `screen.options[].title` / `.text`
      ...
      复验。见下方「6.4c 结论」
- [x] 6.5 **决策日志** `(state, action, 理由)` → `src/mcp_server/journal.py`，
      落在 `logs/decisions.jsonl`（一行一条 JSON，追加写，跨局累积），
      读回用 MCP 工具 `get_journal`。`auto_combat` 的每一步、以及 pick /
      move / choose / play_card / use_potion / proceed 上模型自己写的 `why`
      都进日志。见下方「6.5 结论」
...
#### 6.1+6.2 结论：三个 bug 都是实机跑出来的，不是想出来的

单测（24 项）全绿之后实机跑，仍然抓出三处：

**1. 「杀完还挡不挡得满」这道保险太严，把 §1.1 判反了**

第一版给斩杀加了个前置：杀完之后剩余能量必须还能补齐格挡缺口。结果
strategy.md §1.1 那个实测局面（怪 1 只剩 16 血却要打 16 点）被判成「先堆格挡」
—— 而那一局的结论恰恰是斩杀对。**单测当场抓到了它**，因为用例就是照那个局面写的。
...
改为把**本回合还能叠出来的格挡**算进去。这不会放过 v1 那个死亡局面 ——
HP 37/70 面对 26 点来袭、手上多是废牌时，叠得出的格挡远补不上缺口，照样交还
（已固化成用例）。

**3. 决策日志写了假理由**

起手 5 张全是攻击牌、一张防御都没有，日志却写「格挡已满足，剩余能量输出」。
真相是「该挡但没牌可挡」。**日志写假理由比不写更糟** —— 6.5 的全部价值就在
这个日志上。现在两种情形分得清清楚楚。
...
```jsonc
{"id":"PerfectedStrike","cost":2,
 "values":{"CalculationBase":6,"ExtraDamage":2,"CalculatedDamage":22}}
```

**一个 `Damage` 键都没有** —— 走的是 `CalculatedDamageVar`。这连带两处要改：
桥接层的 `damage_vs` 原本只认 `"Damage"`，对这类牌**算不出目标侧修正**；
启发式也会把牌组里伤害最高的那张当成「不认识的牌」而白白交还。
两边都改成按 `["Damage", "CalculatedDamage"]` 依次取。
strategy.md §1.6 标注「尚未实机核对键名」的那一条，至此核对完毕。

##### 实机验证（2026-08-01，力士，第 5~7 层）

```
SludgeSpinner 39 血     斩杀线净挨 0 vs 堆格挡净挨 4 → 选斩杀，Unrelenting 14 + 打击 6 = 20 正好清掉
三只 CorpseSlug 78 血   4 回合清场，HP 53 → 59（几乎没挨打）
                        回合1 完美打击 22 + 愤怒 6 斩掉 27 血那只，剩 1 能量叠格挡 → 净挨 1
                        回合3 剩下那只是 Debuff 意图 → 一点格挡都不叠（§1.3）
全程                    没打出过一张废牌，没剩过能量
```

##### 边界：它明确不做的事

不用药水、不应答选牌、不碰非战斗界面 —— 一律交还。
手上出现打得起但认不出的牌时**不闷头结束回合**，而是停手交还（要求 3）。
这条会随着认识的牌变多而自然减少触发。

##### ⚠️ 改完 `autoplay.py` 要重启 Claude Code 会话

MCP 工具列表在会话启动时固定。开发期改一版验一版用
`python scripts/autocombat.py` —— 它直连桥接层喂同一份 `autoplay`，
走的是同样的代码路径，只是绕过 MCP 那层壳。

#### 6.4 结论：runner 的难点不是「怎么往下走」，是「什么时候不许自己走」

`decide()` 一共二十来行，顺序即优先级，默认分支是**交还**。它自己只做
唯一解：领金币/遗物/药水、按继续、开宝箱、地图独路、常规战斗（转交
...
`铁斩波`（5 格挡 + 5 伤害）是被当成防御牌选出来的，走了 `enemy_i=None`
那条路，而它的 `target` 是 `AnyEnemy` —— 下发时没带目标，被判 `bad_target`，
整场战斗就此交还。**要不要目标由牌自己说了算，与我们为什么打它无关。**
拿不到更好的目标时默认打威胁最大的那只。

**2. 战斗刚结束的那一瞬间，什么都做不了**

一场打完，奖励界面还没浮出来，此刻是 `NCombatRoom` + 零选项 + 不能继续。
照字面判就是「卡在认不出的界面上，交还」—— 而 0.8 秒后奖励界面就到了。
与 3.4c 那个「等待判据早于目标事件」是同一类错误。修法是把这类局面判成
`unclear` 而非 `handoff`：先重读几次状态，仍是如此才交还。

**3. 「点了没反应」必须能停下来**

商店槽位曾经点了不报错、状态也不变（§3.4f）。runner 撞上这种情况会以每秒
数次的速度空转到上限，故加了局面指纹：连续几步指纹不变即判 `stuck`。
（这条是写的时候就想到的，但真正让它跑起来的是单测里那个假游戏。）

##### 实机验证（2026-08-01，力士 A2，第 9 → 14 层，一路无鼠标）

```
第 9 层  领金币 → 三选一(铁斩波) → 继续 → 走独路 → 宝箱：开箱 + 拿风箱 → 继续
第 11 层 三只小鬼：斩杀两只、结束战斗；奖励只有卡牌 → 交还
第 12 层 岔路(Elite/Monster) → 交还；精英 TerrorEel 140 血：
         打到「预计净掉血 17 > 血量 1/4」→ 停手交还，我接手用击晕线打赢
         战后金币/药水/遗物三样自动领完，只在卡牌三选一上停手
第 13 层 休息点 → 交还（烤火 26→55、最大生命 80→85）
第 14 层 幽灵船：自动打到剩 17 血、触及安全线交还，我一回合斩杀
```

停手点全部落在该停的地方，没有一次替我做构筑或路线决策。

##### 一个已知的保守行为：有斩杀可打时，安全线仍然先触发

第 14 层实测：船只剩 17 血，手上 `完美打击 16 + 突破 6 = 22` 明明能一回合
结束战斗，但安全线（来袭 12 > 血量 1/4）在出牌决策**之前**判，于是照样交还。

这是 6.1 那句「先判安全线，过不了就一张牌都不打」的直接后果，**不是 bug**
（v1 正是死在「一旦接管就一路打到底」）。但确实偏保守 —— 可以改成
「若本回合能清场则不受安全线约束」。暂不动：清场判定要考虑多怪、格挡、
易伤，写错的代价是让安全线出现缺口，而安全线是整个分层的地基。

#### 6.5 结论：日志的价值全在「理由」上，所以理由得由做决定的人写

`auto_combat` 的 log 早就带着动作和理由，缺的是落盘与跨局。补完之后发现，
真正要设计的只有三件事：

**1. 动作名必须在下发之前取好**

日志里记 `pick(2)` 等于没记 —— 下标当场就过期，三天后回头看无从还原。故在
调用前先从局面里把名字取出来（`pick 闪电霹雳`、`move → Monster`、
`play_card Cinder → SewerClam`）。这需要**出手前**的局面，而动作接口只回报
执行后的状态；办法是把上一次调用的返回状态缓存成 `_last_state` ——
上一步的「执行后」正是这一步的「执行前」，一个 HTTP 都不必多发。
...
构筑与路线决策决定一局的上限（strategy.md §3 与 §4），而下一局重开时，
唯一还留得下来的就是这份日志。

**3. 摘要必须筛掉常规操作，但不能筛掉停手记录**

一场战斗二十来条出牌记录，跨局复盘时价值极低。`get_journal` 因此丢掉
`by=heuristic` 且发生在战斗内的记录 —— 但 `auto_combat` **停手交还**那一条
恰好也满足这个条件，而它标出的正是启发式的边界在哪，是最该留下来的一条。
故加了 `kind` 一栏专门标注「这条不是常规操作」。

##### 局的边界：三个 bug 全是单测抓出来的

`/state` 里没有种子一类的局标识（查过 `RunState`，没导出），故由
`logs/current-run.json` 自己维护。三处判据写完即错，单测当场抓住：

| bug | 现象 | 修法 |
|---|---|---|
| 标识只精确到秒 | 死了立刻重开，两局落在同一秒 → **标识撞号，两局混成一份** | 撞号时加序号 `…#2` |
| 「已结束 → 下一条即新局」 | 死亡界面上还要点几下才出得去，那几下被算成了新的一局 | 已结束 + 局面仍是 `game_over` → 仍算上一局；见到**不是** `game_over` 的局面才开新局 |
| 追加写不检查上一行是否完整 | 上一行写到一半被杀进程，下一条直接接在半行后面，**把下一条也毁掉** | 追加前检查末字节是不是换行，不是就先补一个，让损坏止于那一行 |

第三条尤其值得留意：调试期天天在杀游戏、重启 server，写到一半被打断是常态。

##### 实机验证（2026-08-01，力士，第 7~9 层）

单测 19 项之外，六个独立进程连续跑完「奖励 → 三选一 → 继续 → 地图 → 战斗」：

```
pick GoldReward            金币 66→81，日志记的是决策前的 66 ✓
pick 闪电霹雳               名字取自 screen.options[].title，不是下标 ✓
move → Monster             类型名进日志 ✓
play_card Cinder → SewerClam   模型亲自出手那一步，where=combat T1、by=model ✓
auto_combat 5 回合          21 条记录，每条带当时的 HP 与回合数、by=heuristic ✓
get_journal                21 条压成 6 条：构筑 4 + 路线 1 + 停手 1 ✓
```

**局的身份跨了六个进程仍是同一个** —— 这正是它落在盘上而不在内存里的理由：
MCP server 会重启，而一局要打几个小时。

##### 跨局与两个标签：当天晚些时候也验掉了

上面原本记着「跨局、`choose`、`use_potion` 尚未实机验证」。同日两局打完，
三条都有实测了：

```
跨局    两局各自阵亡 → 各写下一条 run_end；digest 给出两条独立战绩
        （两局都到第 17 层）。中间在主菜单的点击归进 menu 桶，不再凭空造局
choose  「choose 痛击+」（癫狂之触指定免费牌）、「choose 羞耻」（商店除卡）
use_potion 「use_potion PowderedDemise」等，均按名字入账
```

**并且当场揪出一个假记录**：`choose 痛击+（2金）` —— `cost` 在商店选项上是
金价，在待答选牌的选项上是**能量费用**，取名器一律按金价渲染。日志写假话
比不写更糟，已按「待答选牌带卡牌 `type`、商店选项没有」分流修掉。

##### 顺带产出：`scripts/call.py`

与 `autocombat.py` 同一个理由 —— MCP 工具列表连同每个工具的参数都在会话启动
时固定，改完 `server.py` 得重启整个会话才看得见新的 `why` 参数。本脚本在另一
个进程里 import 同一份 `server.py` 直接调工具函数，走同样的代码路径。
上面那六个进程就是它跑的。

##### ⚠️ 实机跑出来的一个待修 bug：斩杀判定不看敌人的格挡

验证过程中撞见（第 9 层 `SewerClam`，56 血 + 8 格挡 + `PlatingPower 8`
每回合回满）：

```
回合3  完美打击 22 + 打击 6 = 28 ≥ 26 血  → 判定「斩杀线净挨 0」
实际   两刀先被 8 点格挡吃掉，怪没死，那 14 点照样打在脸上
```

`_killable` 只比 `伤害总和 >= enemy.hp`，**没把 `enemy.block` 算进去**。
危害与当初 `DamageCalc` 那条同型：单向高估自己、低估对手，且敌人格挡越厚
错得越多。

**已修**：斩杀线改为 `hp + block`，并把上面这个局面固化成两条用例
（杀不掉时别装作杀得掉 / 算上格挡仍杀得掉就照杀）。**单测覆盖，实机尚未
复验** —— 复验要等下一次撞上带格挡的敌人，撞上再补记。
...
##### 多段攻击次数：不在统一字段里，而在每张牌的 `WithHitCount(...)`

2026-08-01 实机核对（`双重打击`，卡面「造成5点伤害两次」）：

```
values     {"Damage": 3}     damage_vs [4]      实际掉血 8（打了两次）
```

`values` 里**既没有 `Repeat` 也没有 `Repeats`** —— 6.4b 当初推测的
「照理会以 `Repeat` 键出现在 values 里」不成立，`DynamicVars` 里根本没有
这个变量。而敌人意图那边**有** `repeats`（读的是 `AttackIntent.Repeats`），
两侧不对称。

2026-08-01 重新生成 v0.107.1 的反编译产物后，答案是：`CardModel` **没有统一
的攻击次数属性**，真正结算时由各张牌的 `OnPlay` 把次数传进
`AttackCommand.WithHitCount(...)`。当前 37 处玩家牌调用经静态逐项核对，来源分为：

- 写死的常数（`TwinStrike.WithHitCount(2)`）
- `RepeatVar.IntValue`
- 已在预览管线里算好的 `CalculatedHits`
- 当前能量/星星 X、手牌数、目标是否易伤、本回合是否掉过血等牌面专属公式

桥接层按这份已核对清单导出：固定/全局次数放 `hand[].hits`；像 `Dismantle`
这种随目标变化的次数放 `hand[].hits_vs`。单段牌省略 `hits`，等价于 1；未来
版本里未核对的新牌则不猜，仍按单段保守估算。runner 的伤害统一改成
`(damage_vs[i] 或 values.Damage) × (hits_vs[i] 或 hits 或 1)`。随机目标的多段牌
在多怪场不能保证全部落到指定怪身上，斩杀线仍只计一段；只剩一只怪才全乘。

##### 实机验证（2026-08-01，力士，第 9 层 FossilStalker）

```
出牌前  TwinStrike values.Damage=5  damage_vs=[7]  hits=2
实际    敌人挂易伤，51 → 37，正好 7 × 2 = 14
```

这是 `hits` 字段、目标侧单段伤害与实际扣血的一次完整闭环。其余多段牌的公式
已逐项对过本版本反编译代码，**没有逐张实机打出验证**；不能把静态核对写成
逐牌实测。Python 回归相应增加到 85 项，覆盖固定两段、`hits_vs` 以及随机目标
不得冒充定向斩杀。
...
```
最初只剥标签    耗能变为0res://images/…/ironclad_energy_icon.png。
第一版连图片删掉  耗能变为0。               ← 对这张牌看似正确
但战鼓因此变成    这张牌被消耗时，获得。     ← 数量全在图片个数里
现在              这张牌被消耗时，获得【能量×2】。
```

`StripMarkup` 原本只剥标签、保留标签之间的正文（`[gold]格挡[/gold]` → `格挡`
是对的），但 `[img]` 之间是资源路径，不是正文。第一版为了清掉路径，把整个
`[img]…[/img]` 都删了；这又漏掉了游戏的另一种格式：`energyIcons()` / `starIcons()`
用**图片重复次数本身表示数值**，本地化原文里并没有另一个数字可留。

四次实机遇到的残缺牌（战鼓、跃跃欲试、辉光、放血）全部命中这两个 formatter；
普通的 `{Cards:diff()}` 等数字当时一直正常，所以根因不是 6.4b 那种「没先跑
preview」。现在连续同类图片转成 `【能量×N】` / `【星星×N】`，未知图片也保留
短文件名，不再静默丢语义。
...
##### 图标计数追加实机验证（2026-08-01，力士，第 4、8 层）

同一张此前显示成「获得。」的 `DrumOfBattle`（战鼓）完整走了三条导出路径：

```
商店 screen.options[]   抽2张牌。被消耗时，获得【能量×2】。
买下后 /glossary       同上
打铁 choice.options[]   同上；选择升级后 /glossary 变为【能量×3】
```

三条都是游戏真对象经 `GlossaryExporter.TextOf` / `StripMarkup` 的结果，不是测试
桩。其间另开过多次 `NCardRewardSelectionScreen`，普通数字文本均正常，但随机
奖励没有再次抽到这四张图标计数牌；该界面与商店同走 `TextOf`，没有第三套渲染器。

##### 补：`choice.options[]` 是第二条路，别只补一条

`screen` 那条做完当天就在一个事件（「从 5 张随机牌中选 1 张加入牌组」）上
撞见了 `choice` 那条 —— 它走的是 `CardChoice`，当时仍只给
`id` / `cost` / `type`。候选牌根本不在牌库里，`/glossary` 一样覆盖不到。

```jsonc
// 补之前              // 补之后
{"i":2,"id":"Thunderclap",   {"i":2,"id":"Thunderclap","cost":1,"type":"Attack",
 "cost":1,"type":"Attack"}    "title":"闪电霹雳","text":"对所有敌人造成4点伤害，给予1层易伤。"}
```

同一个渲染器（`GlossaryExporter.TitleOf` / `TextOf`），三行代码。
...
#### 6.3b 结论：测试写在哪一层，决定了它能发现哪一层的错

这条从 6.3 拆出来之后挂了整整一天没做，实现量却只有一个下午。原因不是难，是
**每次开新局都急着往下打** —— 日志攒了好几局，一次都没在开局时被读过。所以
实现里最要紧的不是 `brief()` 抽了哪几个字段，而是 `auto_run` 多出来的那个停手点：
新局第 1 层且本局尚无计划就交还。这与 6.4 同源 —— **该做决策的地方让 runner
停下来**，只不过这次停的是「还没想清楚这一局要改什么」。

落点是 `journal.brief()`（只递上一局硬事实）/ `record_plan()` / `has_plan()`、
MCP 工具 `get_brief` 与 `set_plan`、以及 `decide()` 的开局分支。闭环一行新代码
都没写：`digest()` 既有的筛选规则是「带 `kind` 的一律留」，`kind="plan"` 的记录
于是天然会出现在下一局的 `brief()` 里。全量测试 89 → 121。

同 6.4 的取舍照旧：**停手点排在战斗分支之后** —— 第 1 层直接开打时，半场撂
挑子比漏一次复盘更糟。

但这些都不是这一轮真正的收获。**这个特性反复了四轮才对，而每一轮之所以没被
上一轮的测试抓住，都是因为测试写在了错的那一层。**

**第一轮：功能被写死成永不触发，5 条新用例全绿**

`play_run` 在 `floor <= 1` 时把 `has_plan` 硬编码成 `True`，而 `decide()` 的新
分支**只在** `floor <= 1` 才可能触发 —— 两者相乘等于零。当时新加的 5 条用例
**全部直接调 `decide()`**，一条都碰不到 `play_run`，于是整套测试无异常。更糟的是
那行是为了让一条既有用例变绿才写的：那条用例让假游戏的层数从 5 **倒退**到 1
（现实中 `total_floor` 只增不减），撞上了新分支。**拿正确性换测试通过。**
正确的做法是隔离测试对真实日志的依赖，而不是把功能关掉。

**第二轮：端到端用例把被测的那一侧换成了 lambda**

补的端到端用例把 `journal.has_plan` 整个替换成 lambda，覆盖的是 runner 这一层，
「journal 那一侧算得对不对」仍然零覆盖 —— 漏的还是下面那一层。

**第三轮：真去构造磁盘故障，才炸出「失败即放行」根本没生效**

`has_plan()` 的文档写着「读不到日志就返回 True（失败即放行）」，实际做不到：
`_read_all()` 自己先把 `OSError` 吞掉返回 `[]`，于是 `any([])` 恒为 `False`，
外层 `except` 永远轮不到。后果是磁盘故障时 `record_plan` 也写不进去，模型照着
提示调了 `set_plan` 也救不回来，**`auto_run` 永久卡在第 1 层** —— 一条本该只是
「少复盘一次」的旁路，把整局堵死了。

**第四轮：「失败即放行」只放行了「读」那一半**

日志可读但**写**不进去时，`set_plan` 无条件回报 `ok: True` 谎报成功 →
`has_plan` 恒为 `False` → 同一个死循环换了个入口复现。两处都得改：`set_plan`
如实回报是否真的落盘，`has_plan` 在日志不可写时也放行。

**规矩一：一条新判据要在它自己那一层有用例。** 直接调 `decide()` 只能证明
`decide()` 对；`decide()` 对而调它的人把参数写死，测试照样全绿。凡是把被测对象
换成 lambda 的用例，都要追问一句「那真身谁来测」。而「失败即放行」这类**故障
路径**必须真的构造故障来验，否则验的只是文档注释。

**规矩二：测试变绿不构成修改实现的理由。** 用例与实现打架时，先判是谁错了 ——
上面那条让层数倒退的用例，错的是用例。

##### 一处刻意的取舍：正确性不该借别的函数的副作用

「全新安装（`logs/` 目录不存在）时第 1 层仍然停手」这个正确行为，一度是**借来的**：
它依赖 `has_plan()` 第一句 `run_id()` 会顺手把目录建出来这个副作用，而探针上的
注释还写反了，把修复者本人误导过一次。最后抽出 `_ensure_dir()`（「试着建一次」
而不是「看一眼」），让正确性由这个函数自己保证 —— 调换调用顺序仍然全绿。

##### 同源的一条：决策日志第一次真的用来证伪

同一批工作里重构 `docs/strategy.md`，混进了一条「3 张小刀 = 3 层中毒，与涂毒
相乘」的断言，还被写进了「已验证」。审查者去 `logs/decisions.jsonl` 搜
`刀刃之舞`，4 条命中**全是 `pick`、没有一条 `play_card`** —— 那一局在第 4 层就
结束了，中毒层数一次都没读出来核对过。**这是 6.5 的决策日志第一次真的被用来
证伪一条被当成事实的判断**，而这正是当初做它的理由。规矩：一份靠「只写验证过
的东西」立身的文档，混进一条没打出来过的断言，损害的不是这一条判据本身，而是
读者对全文其余判据的信任。
