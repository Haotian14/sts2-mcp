...
- [x] 2.4b 非战斗状态 → 统一为 `/state` 的 `screen` 段：奖励、卡牌三选一、
      ...
      **除 Boss 遗物外全部已实机验证**（商店 2026-08-01 验证，当场揪出三个
      bug，见 §3.4f 结论）；Boss 遗物必须打赢 Boss 才出现，仍未验
...
- [ ] 3.4g **Boss 遗物三选一**（商店除卡已随 3.4f 一并验证：`pick` 买服务 →
      ...
      **2026-08-01 又打了两局，两局都倒在第一章 Boss 上**，见下方「3.4g 现状」
...
#### 3.4g 现状：两局，两次都倒在第一章 Boss —— 差的不是工具，是牌组
...
**结论与 strategy.md §5 完全一致：牌组强度是上限。** 两次都不是操作失误，
是算得出的数值差距 —— 每回合能投送的伤害追不上它的血量与成长。

工具侧该验的都验了：runner 全程无鼠标跑完两整局、决策日志跨局留下了两局的
战绩与理由。**唯独 Boss 遗物三选一那个界面，至今没有人站上去过。**
它需要的不是再写一行代码，是一套真能打过 Boss 的牌组 —— 这一条从 3.4g
第一次写下时就是这么说的，两局之后依然成立。
