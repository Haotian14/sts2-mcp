...
@server.tool(
    description=(
        ...
        "- `hand[].hits`：攻击次数；`values.Damage` / `damage_vs` 是**单段**伤害，"
        "总伤害要再乘次数。单段牌省略该字段（等于 1）。条件次数按目标不同时用 "
        "`hits_vs`，下标同样与 `enemies[].i` 对齐。\n"
        "- `enemies[].i`：指定目标用的下标。\n"
...
@server.tool(
    description=(
        ...
        "\n"
        "**它绝不替你做的事**（一律停手交还）：\n"
        "- 卡牌三选一、商店、除卡 —— 构筑决策决定一局的上限（strategy.md §3），"
        "上一局整局「拿第一张」的结果是牌组打不动 Boss\n"
        "- 岔路，以及任何通往精英 / Boss 的路\n"
        "- 休息点（烤火还是打铁）、事件（要读文本）\n"
        "- 待答的选牌、认不出的界面\n"
        "- 战斗内触及安全线（血量过低 / 预计掉血过多 / 有致死风险）\n"
        "- 新局第 1 层且本局尚无开局计划 —— 先 get_brief 复盘上一局，"
        "再 set_plan 写下这一局要改什么（spec.md 6.3b），调过之后不再因此停手\n"
        "\n"
        "**返回值的 `stopped` 决定你接下来做什么**：\n"
...
@server.tool(
    description=(
        "读回**多局**决策日志的原始摘要 —— 每条都带当时的层数、血量、做了什么、为什么，"
        "按局分组、逐局逐条列出。\n"
        "\n"
        "**用于跨多局找规律**（比如「最近几局是不是总在同一类界面被迫停手」"
        "「某个构筑思路是不是反复没打穿」），而不是开新局前的例行检查 —— "
        "**开新局前请先用 get_brief**：它只递上一局的硬事实，字段已经按用途"
        "结构化好（死亡层数、停手分组、构筑决策、上一局的计划），比在这里"
        "翻原始条目更省力。\n"
        "\n"
